...
/******** OWNER NUMBER**********/
const ownerNumber = ["18095832598@s.whatsapp.net","18296046446@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Bievenido @${num.split('@')[0]}\ al grupo de *${mdata.subject}* espero que la pases bien!`
				...
				teks = `Ohh saliste @${num.split('@')[0]} jasamu akan di kubur dalam²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                var gh = body.slice(12)
					...
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {caption: 'Listo', quoted: mek})
					await limitAdd(sender)
					break
				//fadli 
				case 'pinterest':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    buff = await getBuffer(`docs-jojo.herokuapp.com/api/text3d?text= {teks}`, {method: 'get'})
			     	await limitAdd(sender)
				break
			    case 'fototiktok':
				if (!isRegistered) return reply(ind.noregis())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    anu = await fetchJson(`https://docs-jojo.herokuapp.com/api/gaming?text={gatauda}` , {method: 'get'})
               ...
               case 'help': 
			    ...
			    case 'jv':
				case 'gxng':
				if (!isRegistered) return reply(ind.anubis())
					client.sendMessage(from, anubis(), text)
					break
				case 'admin':
         	   case 'owner':
         	   case 'creator':
                  client.sendMessage(from, {displayname: "Gxng", vcard: vcard}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Java owner wa.me/+18095832598'
